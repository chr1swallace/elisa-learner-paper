...
```{r}
library(seaborn)
ggplot(pred[type!="Combined"],aes(x=week_num+as.numeric(factor(type))/10,
...
options(digits=6)
kable(probs[Week!="Combined"][order(type,Week),.(type,Week,N,estimate=qm,lower.ci=ll,upper.ci=ul)])
fwrite(probs[Week!="Combined"][order(type,Week),.(type,Week,N,estimate=qm,lower.ci=ll,upper.ci=ul)],
